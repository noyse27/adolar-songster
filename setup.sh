...
port="$default_port"
if [ -n "$port_var" ]; then
  found_port="$(grep "^${port_var}=" "$env_file" 2>/dev/null | tail -1 | cut -d= -f2-)"
  port="${found_port:-$default_port}"
fi
if [ "$mode" = demo ]; then
  echo "Demo: http://localhost:$port - Admin-/Einladungscode siehe Banner in der App, Details in docs/demo.md"
else
  echo "Adolar Songster: http://localhost:$port"
fi
