...
// frontend DemoBanner) - safe only because a demo deployment never holds
// real data and resets on its own schedule. Overridable so an operator
// running several demo stacks can tell them apart, but there is no
// expectation of secrecy here.
...
export const DEMO_ADMIN_PASSWORD = process.env.DEMO_ADMIN_PASSWORD ?? 'songster-demo';
...
// it's printed on the login page by the frontend demo banner.
