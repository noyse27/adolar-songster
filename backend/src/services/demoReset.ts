...
import { DEMO_SONGS } from './demoSongLibrary';
...
// periodically wiping real user/game data.
const DEMO_MARKER_KEY = 'demo_managed';
const DEMO_MARKER_VALUE = '1';

// Every table that holds user-generated activity - reset on each demo
// cycle so a public demo never accumulates real registrations, tables, or
// gameplay history. Listed together (Postgres resolves the FK dependency
// graph across a single TRUNCATE statement's table list on its own) and
// deliberately kept in sync with test/integration/globalSetup.js's own
// TRUNCATE list, plus the tables that list predates (host_device,
// client_debug_event, game_event_log, round_ready, round_sitout,
// round_ready_pref, table_session_song_pool). Deliberately NOT included:
// playboard_reaction (static reference data seeded once by migration,
// unrelated to user activity), adolar_playlist / adolar_playlist_track
// (repopulated by the regular Adolar sync job - irrelevant on a demo
// instance anyway, since ADOLAR_BASE_URL is left unset there). system_setting
// is reset separately below (only the counters that reflect activity, not
// the demo marker itself).
const VOLATILE_TABLES = [
  ...
  'token_usage',
  'guess',
  'session_song_history',
  'round',
  'game_playlist_track',
  'game_playlist',
  'game',
  'table_session_song_pool',
  'table_session',
  'table_seat',
  'game_table',
  'song_ref',
 ...
 * invite code, and the synthetic song library (as source='local' song_ref
 * rows pointing at the ffmpeg-generated clips - see demoSongLibrary.ts and
 * demo/generate-library.sh). Also (re-)writes the demo-managed marker. */
  ...
  try {
    ...
    for (const song of DEMO_SONGS) {
      await client.query(
        ...
        [song.filename, song.title, song.year, song.durationSec, `/demo-songs/${song.filename}`],
      );
    }
...
export function getDemoStatus(): DemoStatus {
  ...
  return {
    ...
    // in the frontend) so the demo banner stays correct even when an
