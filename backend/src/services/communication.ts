...
export type ReactionPhase = 'waiting' | 'countdown' | 'playing' | 'token' | 'resolved' | 'finished';
...
export const REACTION_PHASES: ReactionPhase[] = ['waiting', 'countdown', 'playing', 'token', 'resolved', 'finished'];
...
function emptyReactionConfig(): ReactionConfig {
  return { waiting: [], countdown: [], playing: [], token: [], resolved: [], finished: [] };
}
...
export async function loadReactionPhase(gameId: string): Promise<ReactionPhase | null> {
  ...
  if (row.round_status === 'token_solo' || row.round_status === 'token_others') return 'token';
