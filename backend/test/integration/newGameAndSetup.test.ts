...
describe('POST /setup/self-test (FR-063)', () => {
  it('requires admin auth and reports healthy when db and songpool are fine', async () => {
    ...
    expect(response.body.checks).toEqual({ database: true, songPool: true, roundLogic: true });
  });
});
...
describe('POST /tables/{id}/new-game (AK-009/010/011)', () => {
  ...
  it('keeps the same table_session and player composition, and honors session-wide song history', async () => {
    ...
    const firstRoundSongRow = await pool.query('SELECT song_id FROM round WHERE id = $1', [
      firstRoundId,
    ]);
    const firstSongId = firstRoundSongRow.rows[0].song_id;
    const secondSongId = firstSongId === songAId ? songBId : songAId;
    ...
    // AK-011: the session already played firstSongId, so the only valid
    // candidate left in this session is the other seeded song.
    ...
    const secondRoundSongRow = await pool.query('SELECT song_id FROM round WHERE id = $1', [
      secondRound.body.roundId,
    ]);
    expect(secondRoundSongRow.rows[0].song_id).toBe(secondSongId);
